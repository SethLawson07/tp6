...
public class Crossword extends Grid<CrosswordSquare> {
    ...
    public static Crossword createPuzzle(Database database, int height, int width) throws SQLException {
        ...
        ResultSet resultSet = database.executeQuery("SELECT * FROM CROSSWORD WHERE numero_grille = 1"); // Exemple : charge la grille n°1
    ...
    public boolean isBlackSquare(int row, int column) {
        ...
        // Vérifier si la solution est un espace (case censée être noire)
        if (getSolution(row, column) == ' ') {
            return true;
        }
    
        // Vérifie si la case est couverte par un mot horizontal
        for (Clue clue : horizontalClues) {
            if (clue.getRow() == row && column >= clue.getColumn() && 
                column < clue.getColumn() + clue.getClue().length()) {
                return false; // La case est utilisée dans un mot horizontal
            }
        }
    
        // Vérifie si la case est couverte par un mot vertical
        for (Clue clue : verticalClues) {
            if (clue.getColumn() == column && row >= clue.getRow() && 
                row < clue.getRow() + clue.getClue().length()) {
                return false; // La case est utilisée dans un mot vertical
            }
        }
    
        return true; // Si elle ne fait partie d'aucun mot, elle est noire
    }
    ...
    private boolean[][] correctCells; // Tableau pour suivre les cases correctes
